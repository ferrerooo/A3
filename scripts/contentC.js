...
function init() {
  ...
  function shareDataAcrossAI() {
    console.log(`Claude>>>>>> ${userInput.textContent} is saved to chrome.storage, which is to be shared with others.`);
    chrome.storage.local.set({ whatUserSaid: "Claude>>>>>>" + userInput.textContent });
  }

  function setSubmitFlag() {
    isSubmitBtnClicked = true;
    console.log(`SubmitBtn-click by Claude is saved to chrome.storage, which is to be shared with others.`);
    chrome.storage.local.set({ submittedBy: "Claude_" + new Date() });
  }
